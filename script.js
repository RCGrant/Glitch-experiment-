...
const sent1 = [
  ...
  "Make sure the coffee machine is turned on and in range.",
  "Now discoverable as mildly insecure.",
...
function dropdownFunction() {
  ...
  if (x === "out of comfort zone") {
    addEval("😱👋🦤🥃🧭");
  } else if (x === "angry about nothing in particular") {
    addEval("🔥🌊🌪️🌋🏩");
  } else if (x === "marginally happy") {
    addEval("✌️😌");
  } else if (x === "best to be avoided") {
    addEval("📉🫤❄️⚡🥶");
  } else if (x === "happy") {
    addEval("😄😄😄😄😄");
  }
...
const sevenSyllables = [
  ...
  "thinking about last nights patient",
...
const words = [
  ...
  "indie movies set in the jungle",
...
const subjects =
    "robots, borderlines, displaced animals, right-wing militants, dictators, sailers, part-timers".split(
      ","
    ),
  verbs =
    "wander,search, cross, kneel to pray in, find me in, sneak across enemy lines, hide in the dark side of the moon".split(
      ","
    ),
  objects =
    "the barren plains,the wilted grasslands, deadly wastelands, in the shadow of a giant tree,languishing prisons, arid deserts, military bases, valleys of ash and bone".split(
      ","
    );
// set up the counter
let counter = 0;
...
window.setInterval(processPoem, 3000);